...
An agent never idle-waits on the orchestrator. It ends the turn and is resumed with context intact, or respawned from files if the session died. An implementer that has spawned reviewers does not wait for them by ending its turn: the harness's wake-on-child-completion is unreliable for agent children (2026-09-09, MSITE-265: woken for the spec reviewer, never for the standards reviewer, and the ticket ended with no handback), while a background shell task does re-invoke its owner. It waits with a bounded background shell loop on the verdict files (`until [ -f .deliver/reports/<ticket>-spec-<n>.md ] && [ -f .deliver/reports/<ticket>-standards-<n>.md ]; do sleep 30; done`, capped at the profile's review latency) and acts when that task returns. If an implementer nevertheless ended its turn on a reviewer wait and was not woken, the orchestrator treats the verdict files and the PR as the handback: it finishes the bookkeeping the handback owed (deviation note, tracker state, external-tool threads) and runs the closing round itself rather than respawning.
