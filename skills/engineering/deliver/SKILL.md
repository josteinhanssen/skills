...
- **Review has a shape and a cap.** Plans and specs get one review pass; the planner applies the findings and the orchestrator checks the delta, with a confirm pass only after a Blocking finding. Standards and Spec reviewers on a PR, spawned fresh with refs only, report verbatim; two full passes (one for a `volume: large` ticket), then the orchestrator's closing diff, never a third reviewer spawn, then merge. Mechanical checks (mutation proofs, budget arithmetic, unused-locals, conflict-marker grep, invariants from the profile) are not review findings; they run before review.
